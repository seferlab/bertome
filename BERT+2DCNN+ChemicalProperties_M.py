...
M_ch = pd.read_excel("M.musculus.xlsx")
print(M_ch.head(5))

y_values = M_ch.label
x_values = M_ch.drop(["label"], axis = 1)
...
def CNN_2D():
    ...
    # layer 1
    model.add(Conv2D(32, 3, 3, input_shape=(771, 43, 1), activation="relu"))  # 16,3,3  # 1 Layer ekle bak
...
xtrain_ch, xtest_ch, ytrain_ch, ytest_ch = train_test_split(big_chemical_dataset, y_values, test_size = 0.50, random_state=42)
...
history2_ = model2.fit(np.asarray(xtrain_ch).reshape(len(np.asarray(xtrain_ch)),771,43,1), utils.to_categorical(ytrain_ch,2),
                    validation_data=(np.asarray(xtest_ch).reshape(len(np.asarray(xtest_ch)),771,43,1), utils.to_categorical(ytest_ch,2)),
...
print("---BERT + 2D CNN + Chemical Properties + Accuracy plot was saved.")
...
print("---BERT + 2D CNN + Chemical Properties + Loss plot was saved.")


plt.clf()
import seaborn as sns
from sklearn.metrics import confusion_matrix
cnn_predictions4 = model2.predict(np.asarray(xtest_ch).reshape(len(np.asarray(xtest_ch)),771,43,1))
...
plt.savefig(directory + "/BERT+2DCNNConfusionMatrixwithpwithCh.jpg", dpi = 600)
print("--- BERT + 2D CNN + Chemical Properties confusion matrix plot was saved.")
...
plt.savefig(directory + "/BERT+2DCNNROCCurvewithCh.jpg", dpi = 600)
print("---BERT + 2D CNN + Chemical Properties + ROC Curve plot was saved.")
...
S = pd.read_excel("M.musculus.xlsx")
...
S_ch = pd.read_excel("M.musculus.xlsx")
print(S_ch.head(5))

y_values = S_ch.label
x_values = S_ch.drop(["label"], axis = 1)
...
xtrain_ch, xtest_ch, ytrain_ch, ytest_ch = train_test_split(big_chemical_dataset, y_values, test_size = 0.25, random_state=42)
...
directory = r"C:Usersnisas\OneDrive\Masaüstü\Thesis_Results_Nisa_M"

...
x = np.load("MEMBEDDINGSX.npy")
y = np.load("MEMBEDDINGSY.npy")
...
xtrain, xtest, ytrain, ytest = train_test_split(x, y, test_size=0.20, random_state=42)  #Tree sayısı,  Node ekle ...
...
plt.clf()
plt.plot([0, 1], [0, 1], 'k--')
plt.plot(fpr_keras_bert_rf, tpr_keras_bert_rf, label='AUC (area = {:.3f})'.format(auc_keras_bert_rf))
...
plt.clf()
plt.plot([0, 1], [0, 1], 'k--')
plt.plot(fpr_keras_bert_xgb, tpr_keras_bert_xgb, label='AUC (area = {:.3f})'.format(auc_keras_bert_xgb))
plt.xlabel("False positive rate", fontsize = 15)
plt.ylabel("True positive rate", fontsize = 15)
...
plt.clf()
plt.plot([0, 1], [0, 1], 'k--')
plt.plot(fpr_keras_bert_xgb_hyp, tpr_keras_bert_xgb_hyp, label='AUC (area = {:.3f})'.format(auc_keras_bert_xgb_hyp))
plt.xlabel("False positive rate", fontsize = 15)
plt.ylabel("True positive rate", fontsize = 15)
...
"""
print("-------- BERT + Random Forest Model + Hyperparameter Tuning --------")
...
grid = RandomizedSearchCV(estimator = rfc,param_distributions = random_grid,
               n_iter = 100, cv = cv, verbose=2, random_state=35, n_jobs = -1)
grid_results = grid.fit(xtrain, ytrain)


best_clf = grid_results.best_estimator_
bert_rf_hyp_predictions = best_clf.predict(xtest)


print("Model accuracy score: " + str(accuracy_score(bert_rf_hyp_predictions, ytest)))

f1_score = f1_score(ytest, bert_rf_hyp_predictions)
print("f1_score:", f1_score)

print("Precision:", metrics.precision_score(bert_rf_hyp_predictions, ytest))
print("Recall:", metrics.recall_score(bert_rf_hyp_predictions, ytest))

plt.clf()
from sklearn.metrics import confusion_matrix
confusion_matrix = confusion_matrix(ytest, bert_rf_hyp_predictions)
sns.heatmap(confusion_matrix, annot = True, fmt = "d", cbar = False)
plt.title("BERT + Random Forest + Hyperparameter Tuning Confusion Matrix", fontsize = 20)
plt.savefig(directory + "/BERT+Random Forest+HyperparameterTuningConfusionMatrix.jpg", dpi = 600)
print("---BERT + Random Forest + Hyperparameter Tuning confusion matrix plot was saved.")

from sklearn.metrics import roc_curve
fpr_keras_bert_rf_hyp, tpr_keras_bert_rf_hyp, thresholds_keras_bert_rf_hyp = roc_curve(ytest, bert_rf_hyp_predictions)

from sklearn.metrics import auc
auc_keras_bert_rf_hyp = auc(fpr_keras_bert_rf_hyp, tpr_keras_bert_rf_hyp)
print(auc_keras_bert_rf_hyp)

plt.clf()
plt.plot([0, 1], [0, 1], 'k--')
plt.plot(fpr_keras_bert_rf_hyp, tpr_keras_bert_rf_hyp, label='Keras (area = {:.3f})'.format(auc_keras_bert_rf_hyp))
plt.xlabel('False positive rate', fontsize = 15)
plt.ylabel('True positive rate', fontsize = 15)
plt.title('ROC curve for BERT + Random Forest + Hyperparameter Tuning', fontsize = 20)
plt.legend(loc='best',  prop={'size': 15})
plt.savefig(directory + "/BERT+Random Forest+HyperparameterTuningROCCurve.jpg", dpi = 600)
print("---BERT + Random Forest + Hyperparameter Tuning ROC Curve plot was saved.")
